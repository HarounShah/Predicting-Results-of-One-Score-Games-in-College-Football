...
\usepackage[backend=biber,sorting=nty,sortcites=true,isbn=false,url=false]{biblatex} 
% notes on settings:
...
\title[Down to the Wire: Machine Learning in
One-Score College Football Games]{Down to the Wire: Machine Learning in
One-Score College Football Games} % here, the command format is  \title[Abbreviated Title for Header]{Full Title}. But if your title is short, simply use \title{Full Title}
...
I would like to thank my thesis advisor Dr. Lawrence Seminario-Romero and the faculty in the Department of Mathematics and Statistics at Haverford College for their guidance and support throughout this project. I am also grateful to my family and friends for their encouragement throughout the senior thesis process.
...
It is important that we get a good understanding of all of our features. Firstly, let's observe the numerical summary of each feature. Table \ref{tab:five_number_summary} gives an initial idea of the range and spread of each feature. Most features have varying scales of magnitude, but all have a mean equal to or close to 0. This is to be expected, as any mean away from 0 would hint at some sort of home-field advantage/disadvantage. Overall, nothing from this table seems to stand out or present any concern. Visualizing the features using histograms can help us build upon these initial observations.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.3]{Histograms.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale = 0.3]{WinRateByFeatureBins.png}
...
The top-performing models are SVM and logistic regression by AUC, with gradient boosting close behind; AUC summarizes discrimination across all classification thresholds and is often informative when accuracy alone masks trade-offs between sensitivity and specificity \cite{fawcett2006introduction}. Random forest achieves higher training accuracy but exhibits a larger train-test gap, consistent with flexible tree ensembles that can interpolate idiosyncrasies of the training sample if insufficiently regularized \cite{breiman2001random, hastie2009elements}. Naive Bayes provides a useful probabilistic baseline but trails the other methods on both test accuracy and AUC.

\subsection{Best-Model Diagnostics (SVM)}
Because SVM attains the highest 2025 holdout AUC in Table \ref{tab:model_comparison_2025}, we present its key diagnostic visualizations directly in the main body. The ROC curve in Figure \ref{fig:svm_roc_body} shows strong separation above the random baseline, and the confusion matrix in Figure \ref{fig:svm_cm_body} summarizes class-wise errors at the default classification threshold.

\begin{figure}[h!]
\centering
\includegraphics[width=0.58\textwidth]{\detokenize{Figures/ROC Curves/ROC_SVM.png}}
\caption{SVM ROC curve on the 2025 holdout set.}
\label{fig:svm_roc_body}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.58\textwidth]{\detokenize{Figures/Confusion Matrices/CM_SVM.png}}
\caption{SVM confusion matrix on the 2025 holdout set.}
\label{fig:svm_cm_body}
\end{figure}
\FloatBarrier

\subsection{Feature Importance Across Models}
Feature-importance summaries were generated for each model family (absolute coefficients for linear models, impurity-based scores for tree ensembles), then aggregated into a consensus ranking by averaging feature-rank positions across models. Figure \ref{fig:feature_importance_consensus} shows the resulting consensus pattern.
...
For the highest-AUC model (linear-kernel SVM), the corresponding coefficient-magnitude importance profile is shown in Figure \ref{fig:svm_fi_body}. Large absolute weights again emphasize turnover margin and efficiency variables, consistent with the consensus pattern.

\begin{figure}[h!]
\centering
\includegraphics[width=0.68\textwidth]{\detokenize{Figures/Feature Importance/feature_importance_svm_linear.png}}
\caption{SVM feature importance (absolute linear coefficients).}
\label{fig:svm_fi_body}
\end{figure}
\FloatBarrier
...
The championship game (Indiana vs.\ Miami) provides a useful qualitative example of the same pattern identified earlier in both EDA and model importance rankings: turnover margin can be decisive in close contests. Miami played competitively for most of the game, but a late interception effectively ended a realistic comeback path and shifted the win probability sharply toward Indiana. The game also included a blocked punt, which was another major momentum and field-position swing, even though it is not recorded as a turnover in the core turnover statistics used here. In that sense, this game illustrates why turnover-related features, especially \textit{turnovers\_diff}, consistently appear among the strongest predictors in one-score outcomes, while also highlighting that some special-teams events can be influential but only partially represented in our feature set.
...
Although the models in this thesis are not designed as unconditional game forecasters, they still provide practical value for college football programs. First, the EDA and feature-importance results offer a data-backed framework for self-scouting and postgame review: staffs can prioritize controllable one-score drivers such as turnover margin, passing efficiency, third-down execution, and pressure metrics (e.g., sacks and hurries). Second, the fitted classifiers can be used as scenario-analysis tools during weekly preparation by estimating how shifts in key in-game differentials would change expected outcomes in close-game conditions. Third, these results can support program-level decision making by highlighting which statistical profiles are most consistently associated with winning tight contests, helping align game planning, player development emphasis, and in-game management priorities.

At the same time, there are important limitations. The models were trained on the conditional task of same-game statistics within historical one-score FBS games, so they should not be interpreted as direct predictors of whether a future matchup will be close or who will win outright before the game is played. Football also includes luck, randomness, and many high-leverage events that are difficult to encode with box-score features alone (for example, special-teams swings such as blocked punts). Thus, even with strong methodology, performance ceilings remain modest in close-game settings. These strengths and limitations together reflect the main contribution of this project: machine learning can provide meaningful, actionable structure for understanding one-score outcomes, while still respecting the inherent uncertainty that makes college football competitive and unpredictable.
